...
Pre and post-processing is common when dealing with serialization marshalling.
To support this, Parceler uses callbacks to handle these cases.
A method annotated on a Parceler bean annotated with `@OnWrap` or `@OnUnwrap` will trigger both of these cases respecitively.
...
Parcelable parcelable = Parcels.wrap(example); // CallbackExample.wrap() is called before marshalling.
//...
CallbackExample example = Parcels.unwrap(parcelable); // CallbackExample.unwrap() is called after unmarshalling.
----
